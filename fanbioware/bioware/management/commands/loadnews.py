...
class Command(BaseCommand):
    """Загрузка данных в таблицу News.
    Можно указать путь к csv-файлу. Если путь не указан,
    загрузка выполняется из DEFAULT_FILE_PATH.
    """

    DEFAULT_FILE_PATH = os.path.join(DATA_DIR, 'news.csv')
    IMAGE_DIR = os.path.join(DATA_DIR, 'news')
    help = 'Загружает данные из csv-файла в таблицу News.'
        ...
        with open(file_path, encoding='utf-8') as csv_file:
            ...
            for row in reader:
                ...
                news, created = News.objects.get_or_create(
                    ...
                    text=row['text'],
                    image=uploaded,
                    is_publicated=row['is_publicated'],
                )
                if row.get('game_id'):
                    news.game_id = row['game_id']
                    news.save()
        self.stdout.write(
            self.style.SUCCESS(
